...
public abstract class RestAPIVerticle extends BaseMicroserviceVerticle {
	 ...
	 *            name for our cookie
	 */
	protected void enableLocalSession(Router router, String cookieName) {
		router.route().handler(CookieHandler.create());
		router.route().handler(SessionHandler.create(LocalSessionStore.create(vertx, cookieName)));
	}
	 ...
	 *            name for our cookie
	 */
	protected void enableClusteredSession(Router router, String cookieName) {
		router.route().handler(CookieHandler.create());
		router.route().handler(SessionHandler.create(ClusteredSessionStore.create(vertx, cookieName)));
	}
	...
	protected void requireLogin(RoutingContext context, BiConsumer<RoutingContext, JsonObject> biHandler) {
		Optional<JsonObject> principal = Optional.ofNullable(context.request().getHeader("user-principal")).map(JsonObject::new);
	...
	protected <T> Handler<AsyncResult<T>> resultHandler(RoutingContext context) {
		...
		return ar -> {
			if (ar.succeeded()) {
				T res = ar.result();
				context.response().putHeader("content-type", CONTENT_TYPE_JSON).end(res == null ? "{}" : res.toString());
			} else {
	...
	/**
	 * This method generates handler for async methods in REST APIs. Use the result directly and use given {@code converter} to convert result to string as the
	 * response. The content type is JSON.
	 *
	 * @param context
		...
		return ar -> {
			if (ar.succeeded()) {
				...
				if (res == null) {
					...
					context.response().putHeader("content-type", CONTENT_TYPE_JSON).end(converter.apply(res));
				}
			} else {
				internalError(context, ar.cause());
				ar.cause().printStackTrace();
	...
	/**
	 * This method generates handler for async methods in REST APIs. The result requires non-empty. If empty, return <em>404 Not Found</em> status. The content
	 * type is JSON.
	 *
	 * @param context
	 *            routing context instance
		...
		return ar -> {
			if (ar.succeeded()) {
				...
				if (res == null) {
					...
					context.response().putHeader("content-type", CONTENT_TYPE_JSON).end(res.toString());
				}
			} else {
				internalError(context, ar.cause());
				ar.cause().printStackTrace();
	...
	protected Handler<AsyncResult<Void>> resultVoidHandler(RoutingContext context, int status) {
		return ar -> {
			if (ar.succeeded()) {
				context.response().setStatusCode(status == 0 ? 200 : status).putHeader("content-type", CONTENT_TYPE_JSON).end();
			} else {
				internalError(context, ar.cause());
				ar.cause().printStackTrace();
			}
	...
	/**
	 * This method generates handler for async methods in REST DELETE APIs. Return format in JSON (successful status = 204): <code>
	...
	protected Handler<AsyncResult<Void>> deleteResultHandler(RoutingContext context) {
		return res -> {
			if (res.succeeded()) {
				context.response().setStatusCode(204).putHeader("content-type", CONTENT_TYPE_JSON)
	...
	protected void badRequest(RoutingContext context, Throwable ex) {
		context.response().setStatusCode(400).putHeader("content-type", CONTENT_TYPE_JSON).end(new JsonObject().put("error", ex.getMessage()).encodePrettily());
	}

	protected void notFound(RoutingContext context) {
		context.response().setStatusCode(404).putHeader("content-type", CONTENT_TYPE_JSON).end(new JsonObject().put("message", "not_found").encodePrettily());
	}

	protected void internalError(RoutingContext context, Throwable ex) {
		context.response().setStatusCode(500).putHeader("content-type", CONTENT_TYPE_JSON).end(new JsonObject().put("error", ex.getMessage()).encodePrettily());
	}

	protected void notImplemented(RoutingContext context) {
		context.response().setStatusCode(501).putHeader("content-type", CONTENT_TYPE_JSON)
	...
	protected void badGateway(Throwable ex, RoutingContext context) {
		ex.printStackTrace();
		context.response().setStatusCode(502).putHeader("content-type", CONTENT_TYPE_JSON)
	...
	protected void serviceUnavailable(RoutingContext context, Throwable ex) {
		context.response().setStatusCode(503).putHeader("content-type", CONTENT_TYPE_JSON).end(new JsonObject().put("error", ex.getMessage()).encodePrettily());
	}

	protected void serviceUnavailable(RoutingContext context, String cause) {
		context.response().setStatusCode(503).putHeader("content-type", CONTENT_TYPE_JSON).end(new JsonObject().put("error", cause).encodePrettily());
	}
